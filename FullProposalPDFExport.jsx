...
const FullProposalPDFExport = () => {
  ...
  const optimizeImages = (htmlContent) => {
    // Create a temporary div to parse the HTML
    const tempDiv = document.createElement('div');
    tempDiv.innerHTML = htmlContent;
    
    // Find all images and reduce their quality/size
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        sectionClone.style.height = '297mm'; // A4 height
      ...
      // Save the PDF
      const proposalTitle = proposal.sections.titlePage?.customerName || 'proposal';
      const fileName = `RuM_Project_${proposalTitle.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.pdf`;
      pdf.save(fileName);
...
export default FullProposalPDFExport;
